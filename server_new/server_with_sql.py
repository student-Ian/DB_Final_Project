...
def user_current_order_request(message):
    ...
    Order = get_current_order() #query
    user_orders = [
        {
            "訂單編號": order["OId"],
            "下單時間": order["OrderTime"],
            "騎手取餐時間": order["CollectTime"],
            "到達時間": order["ArriveTime"],
            "終點": order["Destination"],
            "訂單狀態": order["OStatus"],
            "騎手": order["DeliveredBy"],
        }
        for order in Order
...
def orderdetail_request(order_string):
    ...
    for item in items:
        ...
        send_order_detail([oid, rusername, DName, quantity]) # query
...
def menu_request(message):
    ...
    user_dishes = [{"DName": dish["Dname"], "Price": dish["Price"]} for dish in dishes if dish["RUsername"] == username and dish["Dstatus"] == "販售中"]
    return user_dishes  
...
def Voucher_request(message):
    ...
    Voucher_list = [
        {"VCode": voucher["VCode"], "VDiscount": voucher["VDiscount"]}
        for voucher in Vouchers
        if voucher["BelongsTo"] == username
    ]
    return Voucher_list
...
def Coupon_request(message):
    ...
    Coupon_list = [
        {"CCode": coupon["CCode"], "CDiscount": coupon["CDiscount"]}
        for coupon in Coupons
        if coupon["IssuedBy"] == username
    ]
    return Coupon_list
...
def add_dish(message):
    ...
    add_dish([username, DName, str(dish_Price)]) #query
    dishes.append({"RUsername": username, "DName": DName, "Price": str(dish_Price), "Dstatus": "販售中"})

    # 輸出成功消息
    print(f"已成功將菜品 '{DName}' 添加至用戶 {username} 的菜單，價格為 {dish_Price} 元。")
    
    dishes = get_menu_boss()
    user_dishes = [{"DName": dish["DName"], "Price": dish["Price"], "Dstatus": dish["Dstatus"]} for dish in dishes if dish["RUsername"] == username]
    return user_dishes
...
def updated_dishes(message):
    ...
    try:
        ...
        update_dishes([new_name, new_Price, new_status, username, old_name]) #query
        # for dish in dishes:
...
def boss_view_history_order(message):
    ...
    user_orders = [order for order in Order if order["username"] == username]
    return user_orders
...
def handle_view_and_modify_restaurant_info(message):
    ...
    # 假設 restaurant_info 是一個包含字典的列表
        ...
        info = [
            {
                "餐廳名稱": restaurant["RName"],
                "餐廳電話號碼": restaurant["RPhone"],
                "餐廳地址": restaurant["Address"],
                "開始營業時間": restaurant["OpenTime"],
                "結束營業時間": restaurant["CloseTime"]
            }
            for restaurant in restaurant_info if restaurant["RUsername"] == Rusername
        ]
        return info
...
def add_coupons(message):
    ...
    newcoupon = [coupon for coupon in Coupons if coupon["IssuedBy"] == username]
    return newcoupon
